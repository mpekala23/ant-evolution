...
globals [
  ...
  dist-cash ;; distance genetic between queens
]
...
[
  ...
  gen-point ;; genetic point in 3d space list of 3 integers from 1 to 100
  dist-list ;; distance from queen to all queens with lower index
]
...
to setup-queens
  ...
  ask queens [
    set gen-point (list random 100 random 100)
  ]
  ask queens [
    ...
    set color rgb (item 0 gen-point * 1.4) (item 1 gen-point * 1.4) 77
    set dist-list (list )
    let i 0
    while [i < scent-id-count][
      let partners queens with [scent-id = i]
      ifelse any? partners
      [
        let partner one-of partners
        let other-gen [gen-point] of partner
        set dist-list insert-item i dist-list ham-dist gen-point other-gen
      ]
      [
        set dist-list insert-item i dist-list -1
      ]
      set i i + 1
    ]
...
;; Our looping-around hamming like distance for two sequences
to-report ham-dist [seq1 seq2]
  let x1 item 0 seq1
  let x2 item 0 seq2
  let y1 item 0 seq1
  let y2 item 0 seq2
...
to start-fight [others]
  ...
  [
    ...
    [
      ...
      let their-id [scent-id] of mother
      let gen-diff -1
      ifelse my-id < their-id
      [
        set gen-diff item my-id ([dist-list] of mother)
      ]
      [
        set gen-diff item their-id ([dist-list] of matriarch)
      ]

      let i-fight random 100 < gen-diff * fight-mult
      ...
      [
        ...
        [
          ...
          let mat-energy [energy] of matriarch
          let mom-energy [energy] of mother
          if (mat-energy < birth-threshold or mom-energy < birth-threshold)
          [
            ifelse mat-energy < mom-energy
            [
              ask matriarch
              [
                set energy energy + coop-amt
              ]
              ask mother
              [
                set energy energy - coop-amt
...
to split-colony
  ...
  if new-x >= world-width / 2
  [
    set new-x world-width / 2 - 5
  ]
  if new-x <= -1 * world-width / 2
  [
    set new-x -1 * world-width / 2 + 5
  ]
  if new-y >= world-height / 2
  [
    set new-y world-height / 2 - 5
  ]
  if new-y <= -1 * world-height / 2
  [
  ...
  hatch-queens 1 [
    ...
    [
      ...
      [
        set gen-point (replace-item j gen-point (item j gen-point - 100))
      ]
      [
        if item j gen-point < 0
        [
          set gen-point (replace-item j gen-point (100 - item j gen-point))
        ]
      ]
      set j j + 1
    ]
    set color rgb (item 0 gen-point) (item 1 gen-point) 77
    set dist-list (list )
    let i 0
    while [i < scent-id-count][
      let partners queens with [scent-id = i]
...
;; Prints "tick,scent-id,energy,gen-point[0],gen-point[1]"
to print-queen
  file-write ticks
  file-write scent-id
  file-write energy
  file-write (item 0 gen-point)
  file-write (item 1 gen-point)
  file-write -6
  file-print 6
end

;; Netlogo sucks sometimes
to open-necessary
  ifelse run-ix = 0
  [
    file-open "temp_data/0.out"
  ]
  [
    ifelse run-ix = 1
    [
      file-open "temp_data/1.out"
    ]
    [
      ifelse run-ix = 2
      [
        file-open "temp_data/2.out"
      ]
      [
        file-open "temp_data/3.out"
      ]
    ]
  ]
end
